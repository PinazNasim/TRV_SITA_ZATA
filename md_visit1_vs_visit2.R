...
pdf(paste("C://Users//DELL//OneDrive//Documents//md_visit1_vs_visit2.pdf", sep = ""),h= 25,w= 25)
...
for (name in Test_names){
  ...
  plot(c(-37: 7), c(-37: 7), type = "l", 
       ...
       lty = 1, lwd = 2, 
       ylim = c(-33.5,4), xlim = c(-33.5,4), cex.main = 4.5)
  # title(ylab = " MD visit2 (dB)", line = 8, 
  #       xlab = " MD visit1 (dB)", 
  #       cex.lab = 4)
  axis(1, at = seq(5,-35,-10), labels = seq(5,-35,-10),cex.axis= 4,
       padj = 1, lwd = 1.5, col = "black",lwd.ticks = 2)
  axis(2,las = 1, at = seq(5,-35,-10), labels = seq(5,-35,-10)
       ,cex.axis= 4, padj = 0.5, hadj = 1.3, lwd = 1.5, 
       col = "black", lwd.ticks = 2)
  points(Day1, Day2 ,pch = ifelse(DATA$Category == "Normal", 16, 17), cex= 6, col = ifelse(DATA$Category == "Normal", "orangered", "midnightblue"))
  Standard<- cor.test(Day1, Day2, method="spearman")
  text(-28,2, paste("rho =",round(Standard$estimate, digits = 2), sep = " "), cex = 4)
  box(lwd = 2, col = "black")
...
mtext("         MD visit 2 (dB)", side = 4, cex = 4,
      line = 3)
par(mar = c(0, 0, 0, 0))
plot(5, 5, type = "n", xlab = " ", ylab = " ", bty = "n", axes = F)
mtext("                                            MD visit 1 (dB)                                                   ",
      side = 3, cex = 4, line= -10)
legend(3, 5.5, bty = "n", legend = c("Normal", "Glaucoma"), col = c ("orangered", "midnightblue"), pch = c(16, 17), cex = 6)
