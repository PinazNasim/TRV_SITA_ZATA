...
png("C://Users//DELL//OneDrive//Documents//TRV_bins_hist.png",
    family = "Times", h = 4500, w= 4500)

layout(matrix (c( c(21:26), c(1:5), 26, c(6:10),26 , c(11:15),26,
                  c(16:20), 26), ncol = 5, byrow = F)
       ,heights = c(0.35,rep(1, 3), 1.1, 0.25), widths = c(0.75, rep(1, 4)))
...
for (t in Test){
  ...
  for(participant in 1:77){
    for(threshold in 0:33){
      ...
      x[[i]] <- c(x[[i]],as.numeric(DATA[participant,CD2[location]]))
  ...
  for(n in 1:4){
    ...
    # ifelse(n == 4,par(mar = c(15, 5, 5, 3)) , par(mar = c(10, 5, 5,3)))
    ...
    axis(1, at= seq(0, 35, by =5), labels = seq(0, 35, by =5),line = 1,
         cex.axis = ifelse(n == 4, 12, 0.001), 
         lwd = ifelse(n == 4, 2, 0.001), 
         col= ifelse(n == 4, "black", "white") 
         ,padj = 2)
    points(ifelse(n == 1, which(s$counts > 160)- 0.35, 1),153, pch = 17,
    ...
    points(ifelse(n == 4 & test == "ZS", 
                  which(s$counts > 1000)- 0.35, 1),970, pch = 17,
           col = ifelse(n == 4 & test == "ZS", "black","white" ),cex = 15)
    text(ifelse(n == 4 & test == "ZS", 
                which(s$counts > 1000)- 8, 1),950,
...
for (n in 1:4){
  ...
  plot(5, 5, type = "n", xlab = " ", ylab = " ", axes = F)
  text(5, 5 , paste( Label[n]," dB", sep = " "), cex = 15, srt = 90)
  
  
}
mtext("                                                         Frequency Distribution",
      side = 2, cex = 15, line = 1)

plot(5,5 , type = "n", xlab = " ", ylab = " ", axes = F)

mtext("                 Threshold (dB)", 
      side = 3, cex = 15, line = -15)
